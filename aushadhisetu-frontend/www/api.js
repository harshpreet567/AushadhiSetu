/* ============================================================
   ...
   Authentication does NOT fall back to mock data.
   Other prototype APIs can still use mock data if backend is offline.
   ============================================================ */
...
const api = {
  ...
  // LOGIN / SIGNUP
  // IMPORTANT:
  // Authentication NEVER falls back to mock data.
  // ==========================================================
  ...
  async login(payload, mode = "login") {
    ...
    try {
      ...
      // Backend must provide an access token
      if (!data.access_token) {
        throw new Error(
          "Authentication succeeded but no access token was returned by the backend."
        );
      }
      ...
      console.error(
        "AushadhiSetu authentication error:",
        error
      );

      // VERY IMPORTANT:
      // Do NOT return DB.facility here.
      // Login must fail if backend authentication fails.
  ...
  async getDashboard() {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock dashboard:",
        e
      );
    ...
    return {
      total_stock: meds.reduce(
        (s, m) => s + m.quantity,
        0
      ),

      low_stock: meds.filter(
        m => m.days_of_stock <= 5
      ).length,

      expiring_soon: meds.filter(
        m =>
          m.expiry_risk === "HIGH" ||
          m.expiry_risk === "MEDIUM"
      ).length,

      surplus_count: meds.filter(
        m => m.potential_surplus > 0
      ).length,

      top_alert: DB.alerts[0] || null,

      top_recommendation:
        DB.recommendations[0] || null
    };
  },
  ...
  async getInventory() {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock inventory:",
        e
      );
  ...
  async getMedicine(id) {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock medicine:",
        e
      );
  ...
  async getAlerts() {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock alerts:",
        e
      );
  ...
  async getSurplus() {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock surplus:",
        e
      );
  ...
  async getFacilities() {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock facilities:",
        e
      );
  ...
  async getRecommendations() {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock recommendations:",
        e
      );
  ...
  async createTransfer(recommendation_id) {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock transfer creation:",
        e
      );
  ...
  async initiatePayment(transfer_id) {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock payment settlement:",
        e
      );
  ...
  async getTransferStatus(transfer_id) {
    ...
    try {
      ...
      console.warn(
        "Backend unavailable, using mock transfer status:",
        e
      );
